...
void
round_robin::spawn( fibers::detail::fiber_base::ptr_t const& f)
{
    BOOST_ASSERT( f);
    BOOST_ASSERT( f->is_ready() );

    // store active fiber in local var
    fibers::detail::fiber_base::ptr_t tmp = active_fiber_;
...
round_robin::run()
{
    ...
    {
        fibers::detail::fiber_base::ptr_t f( s.f);

        BOOST_ASSERT( ! f->is_running() );
        BOOST_ASSERT( ! f->is_terminated() );

        // set fiber to state_ready if dead-line was reached
        if ( s.tp <= fibers::clock_type::now() )
            f->set_ready();
...
void
round_robin::wait( unique_lock< fibers::detail::spinlock > & lk)
{ wait_until( fibers::clock_type::time_point( (fibers::clock_type::duration::max)() ), lk); }

bool
round_robin::wait_until( fibers::clock_type::time_point const& timeout_time,
                         unique_lock< fibers::detail::spinlock > & lk)
{
    fibers::clock_type::time_point start( fibers::clock_type::now() );
    ...
    BOOST_ASSERT( tmp == fibers::detail::scheduler::instance()->active() );
    BOOST_ASSERT( tmp->is_running() );

    return fibers::clock_type::now() < timeout_time;
}
...
void
round_robin::join( fibers::detail::fiber_base::ptr_t const& f)
{
    BOOST_ASSERT( f);
    BOOST_ASSERT( f != active_fiber_);
...
void
round_robin::priority( fibers::detail::fiber_base::ptr_t const& f, int prio) BOOST_NOEXCEPT
{
    BOOST_ASSERT( f);
